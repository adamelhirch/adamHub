...
recipe/mealplan/goal), pre-existing rows have user_id = NULL and are invisible
to everyone. This script claims them for the account whose email is given
with --email (which must already exist — register it first via POST
/auth/register).
...
from app.models import Goal, GroceryItem, MealPlan, PantryItem, Recipe, User

BACKFILL_MODELS = [GroceryItem, PantryItem, Recipe, MealPlan, Goal]
