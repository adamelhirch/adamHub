...
from app.api._crud import apply_updates, create, get_owned_or_404
...
def create_habit(payload: HabitCreate, session: SessionDep, user: CurrentOrOwnerUser) -> HabitRead:
...
def list_habits(
    ...
    statement = select(Habit).where(Habit.user_id == user.id).order_by(Habit.created_at.desc())
    if active_only:
...
def update_habit(
    ...
    habit = get_owned_or_404(session, Habit, habit_id, user_id=user.id, detail="Habit not found")
...
def log_habit(
    ...
    log = create(session, HabitLog(habit_id=habit_id, user_id=user.id, **payload.model_dump()))
...
def list_habit_logs(
    ...
    get_owned_or_404(session, Habit, habit_id, user_id=user.id, detail="Habit not found")

    logs = session.exec(
        select(HabitLog)
        .where(HabitLog.habit_id == habit_id, HabitLog.user_id == user.id)
