...
def create_note(payload: NoteCreate, session: SessionDep, user: CurrentOrOwnerUser) -> NoteRead:
    note = create(session, Note(**payload.model_dump(), user_id=user.id))
...
def list_journal_entries(
    ...
    statement = (
        select(Note)
        .where(Note.kind == NoteKind.JOURNAL, Note.user_id == user.id)
...
def get_note(note_id: int, session: SessionDep, user: CurrentOrOwnerUser) -> NoteRead:
    note = get_owned_or_404(session, Note, note_id, user_id=user.id, detail="Note not found")
    return NoteRead.model_validate(note, from_attributes=True)


@router.patch("/{note_id}", response_model=NoteRead)
def update_note(note_id: int, payload: NoteUpdate, session: SessionDep, user: CurrentOrOwnerUser) -> NoteRead:
    note = get_owned_or_404(session, Note, note_id, user_id=user.id, detail="Note not found")
...
def delete_note(note_id: int, session: SessionDep, user: CurrentOrOwnerUser) -> dict:
    note = get_owned_or_404(session, Note, note_id, user_id=user.id, detail="Note not found")
    delete(session, note)
